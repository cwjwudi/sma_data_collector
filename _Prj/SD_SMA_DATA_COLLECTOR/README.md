...
- 只有某个触发索引的全部配置数据点均读取成功且该行成功进入内存队列后，程序才复位该索引。
- 复位会通过 OPC UA 读回确认；若复位后 PLC 已立即再次置位，程序会保留该高电平并在下一轮作为新事件采集。
- 触发复位表示采集器已将完整数据接收到内存 `deque`，不表示 MySQL 已经提交。
- MySQL 瞬时失败的批次保留在内存重试队列，不可自动重试的数据保留在内存 dead-letter 队列；正常停机会尝试刷新可重试队列。断电或进程崩溃仍可能丢失内存数据，持久化队列将在后续版本提供。
