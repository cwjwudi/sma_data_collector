"""web_config 远程访问 token 鉴权（loopback 放行 / 非 loopback 需 X-SD-SMA-Token）。"""
...
from web_config.main import app
...
def _get(source: tuple[str, int], path: str, headers: dict[str, str] | None = None):
    # 不进入上下文管理器，避免触发 lifespan 的采集器自动启动逻辑。
    client = TestClient(app, client=source)
...
def test_static_not_exempt_for_non_loopback(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv(TOKEN_ENV, raising=False)
    assert _get(REMOTE, "/static/home.html").status_code == 403
