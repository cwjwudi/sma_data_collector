"""Encrypt database and OPC UA credentials before unified profiles are persisted."""
...
KEY_FILENAME = ".sd_sma_query_web_fernet.key"
...
def decrypt_secret(config_dir: Path, token: str | None, label: str) -> str:
    if not token:
        return ""
    try:
        ...
        raise ValueError(
            f"{label}密码密文无法解密，请确认 {config_dir / KEY_FILENAME} 与配置文件匹配，"
            "或重新输入密码保存配置"
        ) from exc
...
def prepare_password_mapping(
    ...
    result.pop("password_configured", None)
...
def resolve_password(
    ...
    *,
    ...
    if data.get(ENCRYPTED_FIELD):
        return decrypt_secret(config_dir, str(data[ENCRYPTED_FIELD]), label)
