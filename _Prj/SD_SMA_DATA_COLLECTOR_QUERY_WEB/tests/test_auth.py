"""远程访问 token 鉴权（loopback 放行 / 非 loopback 需 X-SD-SMA-Token）。"""
...
def _get(source: tuple[str, int], path: str, headers: dict[str, str] | None = None):
    # 不进入上下文管理器，避免触发 lifespan 的 OPC UA 监视任务。
...
def test_health_exempt_for_non_loopback(monkeypatch: pytest.MonkeyPatch) -> None:
    # /api/health 会探测数据库，测试环境下可能返回 503，但不应被鉴权拦成 403。
    monkeypatch.delenv(TOKEN_ENV, raising=False)
    assert _get(REMOTE, "/api/health").status_code != 403
