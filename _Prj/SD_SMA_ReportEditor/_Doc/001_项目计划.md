...
> **交付格式（现行）：** 报表成品为 **PDF**（Electron 渲染导出），模版与配置为 **JSON**。初版设想的 Markdown 生成与 `data/history/*.md` 归档 **已取消**，理由见 [005_交付格式决策-取消Markdown管线.md](005_交付格式决策-取消Markdown管线.md)。

## 技术架构（现行）

```mermaid
flowchart TD
    Electron["Electron 桌面壳"] -->|"localhost HTTP"| FastAPI["Python FastAPI"]
    Electron --> Vue["Vue 3 前端"]
    FastAPI --> DB["数据库连接 / 只读查询"]
    FastAPI --> OpcUA["OPC UA 客户端"]
    FastAPI --> Tpl["模版 / 版式 / 签名存储"]
    FastAPI --> Cfg["配置导入导出"]
    Vue --> Preview["导出预览栈\n与 PDF 同源"]
    Electron --> PDF["用户目录\n输出 .pdf"]
    OpcUA -->|"参数"| DB
...
│   └── package.json
├── rptp/                     # 报表模板/版式原型（Vite+TS，可选；见根 README）
├── scripts/                  # 开发启停（dev/windows、dev/mac）
├── packaging/                # Win/Mac 安装包打包
├── getting-started/          # 初次上手文档
└── README.md                 # 完整目录说明见 README「目录结构」一节
```
...
- **报表生成（现行）**：导出时由前端预览栈 + 后端填数（OPC UA / SQL）渲染为 **PDF**；不经过「拼接 Markdown 文件」步骤（见 [005](005_交付格式决策-取消Markdown管线.md)）
- **历史报表（现行）**：`ReportHistory` 列举用户指定的**导出文件夹**中的 PDF，而非 `data/history/*.md`
- **config 导入导出**：全量配置（数据源、OPC UA、模版等）序列化为 JSON 包，支持跨机迁移
...
- `history/`：目录仍会初始化；**不再**存放生成的 `.md` 报表（成品 PDF 由用户选择导出路径）
...
| **TemplateEditor** | 可视化模版编辑：版式区带、绑定 OPC/SQL、导出预览（与 PDF 同源） |
| **ReportGenerator** | 选手动或 OPC 条件触发 → 导出 **PDF**（Electron） |
| **ReportHistory** | 监视导出文件夹中的 **PDF** 列表与预览 |
| **LayoutPresets / Signatures** | 版式预设、签名库（计划外已交付，见 002 文档 M+） |
...
- 开发阶段：`npm run dev` + `uvicorn`，或 `scripts/dev` 一键脚本；Electron 用 `npm run electron:dev`
- 打包阶段：**Windows NSIS**、**macOS DMG**（`packaging/`）；内嵌 PyInstaller 后端，详见 [packaging/README.md](../packaging/README.md)
- 报表成品：**PDF** 文件（非 `.md`）
