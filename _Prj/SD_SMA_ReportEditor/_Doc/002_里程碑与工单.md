...
| M1-T2 | Python 后端初始化：FastAPI + main.py 空壳 + requirements.txt + data/ 自动创建 | 可启动的 FastAPI 服务 | `uvicorn main:app` 启动成功，访问 `/docs` 可见 | ✅ 完成 |
| M1-T3 | 前端初始化：Vite + Vue 3 + vue-router + pinia 脚手架 | 可启动的 Vue 开发服务器 | `npm run dev` 启动成功，浏览器可见空页面 | ✅ 完成 |
| M1-T4 | Electron 主进程：main.js 启动 Python 子进程 + 加载前端页面 | Electron 窗口启动 | 双击运行能弹出窗口，内嵌前端页面，后端同时启动 | ✅ 完成 |
| M1-T5 | 全局布局：侧边栏导航 + 7 个页面空壳路由 + Dashboard 快速入口卡片 | 完整导航框架 | 点击侧边栏各菜单能切换到对应空页面 | ✅ 完成 |
...
| 2026-03-10 | M1-T1 完成：目录结构、README.md |
| 2026-03-10 | M1-T2 完成：FastAPI 后端 main.py + data/ 自动初始化 |
| 2026-03-10 | M1-T3 完成：安装 Node.js v24.14.0，npm install 安装 Vue 3 依赖，npm run dev 验证通过（Vite v6.4.1 @ localhost:5173） |
| 2026-03-10 | M1-T4 完成：Electron 主进程改进（venv 自动检测、后端健康轮询、concurrently 并行启动），npm run electron:dev 验证通过 |
| 2026-03-10 | M1-T5 完成：侧边栏导航 + 7 个页面路由 + Dashboard 快速入口卡片，截图验收通过 |
| 2026-03-10 | **M1 里程碑全部完成** |
