...
SYSTEM_PROMPT = (
    ...
    "新建用 create_blank_*；删除必须 delete_template / delete_layout_preset（确认流）；"
    "若返回 awaiting_user_confirm，须提示用户在本机弹框确认，禁止声称已删除；取消则资产仍在；"
    "排序 set_template_display_order；打开编辑用 request_open_*。"
    "备份/导入/复位：导出备份必须 request_config_backup_export（返回 awaiting_user_action，"
    "提示用户在本机弹框另存 .rebak；禁止声称已生成文件内容或口令）；"
    "merge 导入 request_config_import_merge；复位 request_config_reset；"
