...
!macro customUnInstall
  ...
  ${ifNot} ${isUpdated}
    IfFileExists "$APPDATA\sd-sma-report-editor\*.*" 0 done_userdata
      ...
      MessageBox MB_YESNO|MB_ICONQUESTION "是否在卸载前备份本软件数据（数据源、模版、版式、签名、设置）？$\r$\n$\r$\n选择“是”将复制到「文档」文件夹，便于重装后恢复。$\r$\n（如需加密备份，请取消卸载，先在软件「设置 → 备份与恢复」导出加密备份。）" IDNO do_delete_userdata
        ; 备份目标：我的文档\ReportEditor-Backup
        StrCpy $0 "$DOCUMENTS\ReportEditor-Backup"
        CreateDirectory "$0"
        nsExec::ExecToLog 'cmd /c xcopy /E /I /Y /Q "$APPDATA\sd-sma-report-editor\backend-data" "$0\backend-data"'
        Pop $1
        ${If} $1 == 0
