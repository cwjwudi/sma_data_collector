  ...
  <div class="lpep">
    ...
    <div class="lpep-grid">
      ...
      <template v-if="el.type === 'table'">
          ...
          <div class="lpep-dim-field">
            ...
            <div class="lpep-dim-stepper" role="group" aria-label="表格行高">
              <button
                ...
                title="减小行高"
                aria-label="减小行高"
                :disabled="tableRowHeightModel <= TABLE_ROW_HEIGHT_MIN_PX"
                @click="bumpTableRowHeight(-1)"
              >
                −
              </button>
              <input
                v-model.number="tableRowHeightModel"
                type="number"
              ...
              <button
                ...
                title="增大行高"
                aria-label="增大行高"
                :disabled="tableRowHeightModel >= TABLE_ROW_HEIGHT_MAX_PX"
                @click="bumpTableRowHeight(1)"
              >
                +
              </button>
        ...
        <div v-if="el.type === 'table'" class="lpep-table-sql-fill-block">
          <div class="lpep-sql-fill-row">
            <span class="lpep-sql-fill-title">数据库填充</span>
            <button
              type="button"
              class="lpep-switch"
              :class="{ 'lpep-switch--on': tplSqlFillEnabled }"
              role="switch"
              :aria-checked="tplSqlFillEnabled"
              :disabled="tplSqlFillSwitchLocked"
              @click="onTplSqlFillToggle"
            />
          </div>
          <p v-if="tplSqlFillSwitchLocked" class="lpep-hint-muted">
      ...
      <template v-if="el.type === 'signature'">
        <label class="lpep-lab">签署说明（画布上以浅色描摹水印显示）<input v-model.trim="el.signerLabel" class="lpep-inp" placeholder="如：张三·签收" /></label>
        <label class="lpep-lab"
          >阅览显示<select v-model="signatureDisplayModeChoice" class="lpep-inp">
            <option value="watermark">仅水印（签署说明描摹）</option>
            <option value="handwriting">仅手写图</option>
            <option value="both">水印与手写同时显示</option>
          </select></label
        >
        <label class="lpep-lab"
          >签名库<select
        ...
        >
        <button type="button" class="lpep-file-btn" @click="emit('open-signature-pad')">手写录入…</button>
        <p class="lpep-hint-muted">
          若已选签名库，打开手写板时会自动显示<strong>浅色库图水印</strong>与条目名描摹线；确定后若有墨色笔画则<strong>自动叠加</strong>在库图之上，未落笔则仅保存库图。
        </p>
      </template>
...
import type { SignatureDisplayMode, TemplateControlType, TemplateElement, TemplateTableCell } from "@/lib/report-template/model";
import {
  ensureTableGrid,
...
const signatureDisplayModeChoice = computed<SignatureDisplayMode>({
  get() {
    if (props.el.type !== "signature") return "both";
    return normalizeSignatureDisplayMode(props.el.signatureDisplayMode);
  },
  set(v: SignatureDisplayMode) {
    if (props.el.type === "signature") props.el.signatureDisplayMode = v;
  },
});
...
function ensureTplTableSqlFill(el: TemplateElement): TableSqlFillConfig {
...
function ensureElementSqlParams(el: TemplateElement): TableSqlParamBinding[] {
  if (!Array.isArray(el.sqlParams)) el.sqlParams = [];
  ensureSqlParamSlots(el.sqlParams, 2);
  return el.sqlParams;
}

function ensureTableCellSqlParams(cell: TemplateTableCell): TableSqlParamBinding[] {
...
function openTableCellSqlParamOpcPicker(slot: number) {
  const cell = activeTableCell.value;
  if (!cell) return;
  ensureTableCellSqlParams(cell);
  opcPickTarget.value = {
    kind: "scalarSqlCell",
    row: editCellRow.value,
...
watch(
  ...
  () => {
    ...
    tableDimRows.value = props.el.tableRows ?? 3;
    tableDimCols.value = props.el.tableCols ?? 4;
  },
  { immediate: true },
);
...
watch(
  ...
  (rows) => {
    if (props.el.type !== "table" || rows == null) return;
    if (tableDimRows.value !== rows) tableDimRows.value = rows;
  },
);

watch(
  () => [props.tableCellPick ?? null, props.el.id, props.el.type] as const,
  ([pick, id, typ]) => {
    if (typ !== "table") return;
    if (pick && pick.elId === id) {
...
function onDateFormatPresetChange(ev: Event) {
  ...
  const el = props.el;
  if (el.type !== "date" || v === "__custom__") return;
  el.dateFormat = v;
}

const activeTableCell = computed(() => {
  if (props.el.type !== "table") return null;
...
const tplAnyCellBinding = computed(() => {
  ...
  ensureTableGrid(props.el);
  return gridHasNonNoneBinding(props.el.tableCells);
});

const tplSqlFillSwitchLocked = computed(() => tplAnyCellBinding.value && !tplSqlFillEnabled.value);

function onTplSqlFillToggle() {
  const el = props.el;
  if (el.type !== "table") return;
  const cfg = ensureTplTableSqlFill(el);
  if (cfg.enabled) {
    cfg.enabled = false;
    return;
  }
...
const tableRowHeightModel = computed({
  get(): number {
    if (props.el.type !== "table") return TABLE_ROW_HEIGHT_DEFAULT_PX;
    return clampTableRowHeightPx(props.el.tableRowHeightPx);
  },
  set(v: number) {
    if (props.el.type !== "table") return;
...
function onOpcPickConfirm(payload: string | { serverId: string; nodeId: string }) {
  ...
  if (typeof t === "object" && t?.kind === "scalarSqlParam") {
    const params = ensureElementSqlParams(props.el);
  ...
  if (typeof t === "object" && t?.kind === "scalarSqlCell" && props.el.type === "table") {
    const cell = ensureTableGrid(props.el)[t.row]?.[t.col];
    if (!cell) return;
    const params = ensureTableCellSqlParams(cell);
    ensureSqlParamSlots(params, t.slot + 1);
    const row = params[t.slot];
    if (row) {
      row.source = "opcua";
