...
:run_build
echo.
echo == Report Editor AI Windows Build ==
echo Tip: run "git pull" in repo first; version from frontend\package.json ^(target 0.2.5^).
...
if not "%EXITCODE%"=="0" (
  ...
  echo   7. Version mismatch - bump package.json / latest.json, then: build.bat -Fresh
  echo   8. Current version Setup missing - use -Fresh ^(older Setup-*.exe are kept^)
  echo.
