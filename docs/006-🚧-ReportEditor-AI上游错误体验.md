...
# ⌛️ 未完成：排队改到输入框上方收纳（类 Cursor · 不入消息流）
...
| Q2 | 消息列表是否仍渲染 `queued` | **否**；`messages` 不再插入 `status=queued` 气泡；队列只活在 `chat-queue` + 收纳条 |
| Q3 | 出队瞬间 | 收纳条去掉该项 → 消息流追加正式 user 气泡 → 开始助手流式回复（与现 `pumpQueue` 时序一致） |
| Q4 | 空闲且队列空 | **整块收纳条隐藏**（不占位） |
| Q5 | 暂停 / 上限 / 单条取消 | 行为不变（G2/G3/F8）：暂停文案可挂在收纳条旁；× 仍只删该条 |
| Q6 | 持久化 | 排队仍**默认不持久化**（与 014 一致）；刷新后队空 |
| Q7 | 本条不做 | 流式「先工具后正文」、多轮啰嗦压缩（另条登记） |

## 拟改落点

1. `AiDrawer.vue`：消息 `v-for` 过滤掉 queued；composer 上方新增收纳条绑定 `queue`。  
2. `onSend` / `pumpQueue`：入队只改 `queue`，不 `push` queued 气泡；出队再 `push` user `done`。  
3. `chat-queue.ts` 可保持；补 UI 单测或组件测（可选）。  
4. 对照修订 [docs/014](014-✅-ReportEditor-AI流式输出.md) 中「排队气泡进消息流」的旧约定。
...
| T2 | 当前轮结束 | 队首出队 → 正式 user 气泡出现在流末 → 开始回答；收纳剩余 N-1 |
| T3 | × 取消某条 | 仅该条从收纳消失；不影响当前流 |
| T4 | 队列空 | 收纳条消失 |
