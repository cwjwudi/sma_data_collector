...
> **Agent 体验（2026-07-13 补充）**：对话须可见工具调用与状态；口头结论必须与工具结果一致，失败须重试或如实失败——见 H1「Agent 可见工具轨迹与结论闭环」。

---

# ⌛️ 未完成：Agent 可见工具轨迹与结论闭环（探活为例，能力域通用）

## 产品诉求（用户举例 · 2026-07-13）

以「定时探活」为例（其它写入/确认类能力同标准）：

1. 用户要求开启探活（或问「探活开了吗」）。  
2. 若助手回复「已经开启 / 探活已打开」——则本机偏好与设置开关**必须已经是开**；不得只聊天答应。  
3. 若工具失败、未调工具、或读回仍为关——**不得**说已开启；应在本轮内**重试工具**或明确失败原因（总闸关、工具禁用、上游错误等）。  
4. 对话框里要能**准确看到**：调用了哪个工具、参数要点、成功/失败状态（及简短结果），达到一定程度的 **Agent 可观测性**，而不只是一段自然语言。

## 现状差距（代码对照）

| 层 | 现状 | 缺口 |
|----|------|------|
| 后端 `run_chat_completion` | 已有最多 `MAX_TOOL_ROUNDS=8` 的 tool loop，工具结果回灌 LLM | 返回给前端的仍是上游最终 `choices[0].message` 文本；**未单独下发 tool 轨迹** |
| 前端 `AiDrawer` | 只展示 `extractAssistantText` 的助手正文 | **无**工具名 / 轮次 / ok·error 时间线 |
| 结论一致性 | SYSTEM_PROMPT 已要求探活必调工具、`ok=false` 如实告知 | 缺少「声称已开启前须有成功工具结果（或读回确认）」的硬约束与 UI 证据 |
| 失败重试 | 依赖模型在 tool loop 内自觉再调 | 未对「用户意图未达成」做服务端校验后再多一轮；用户看不见是否重试过 |

## 期望行为（Agent 最小闭环）

```text
用户意图（开探活）
  → 调 update_connection_probe_settings(enabled=true)  [可见：调用中]
  → 工具结果 ok / 读回 connection_probe_enabled     [可见：成功/失败]
  → 若失败：同轮再试或停止并说明原因               [可见：重试]
  → 仅当成功后，助手文案才可写「已开启」
  → mirror 刷新设置页开关
```

写「已是开启」时：须先 **get/读偏好或本轮工具 applied** 证明已是 true；不得空口。

## 拟改（确认后开工 · 可分切片）

### A. 对话框工具轨迹（前端 + API）

1. 聊天响应增加 `tool_trace`（或等价）：每步 `{ name, args_summary, ok, message/error, round }`（**禁止**把密码/备份密文放进 trace）。  
2. `AiDrawer` 在助手气泡旁或下方渲染可折叠「工具调用」列表：状态色（进行中/成功/失败）。  
3. 可选：进行中显示「正在调用 update_connection_probe_settings…」。

### B. 结论与事实对齐（后端策略）

1. 对探活等关键意图：若最终 assistant 文本声称已开启，但本轮无成功 write/读回为 true → 追加一轮纠错或改写为失败说明（产品拍板：拦一句 vs 强制再调工具）。  
2. 工具 `ok=false` 时：提示模型必须重试一次或转述错误；前端用红色失败态展示。  
3. 单测：模拟「未调工具却说已开」的响应路径（若做服务端校验）。

### C. Agent 能力程度（本看板边界）

| 要 | 不要（本阶段） |
|----|----------------|
| 多轮 tool loop（已有上限）+ 轨迹可见 | 完全自主规划无关长任务 / 无人值守改生产配置 |
| 写入类「说做必做 / 做不成就说清楚」 | 绕过确认弹框、绕过写入总闸 |
| 用户能核对工具名与成败 | 把完整上游 JSON / 密钥甩进对话框 |

## 验收（实现后）

1. 说「开启定时探活」→ 对话框出现对应工具调用；成功则设置开关为开，文案与之一致。  
2. 总闸关闭再说一次 → 可见工具失败；文案不得声称已开启。  
3. 人为模拟工具失败后，可见重试或明确停止原因。  
4. 能力域表中其它 write/confirm 项复用同一轨迹 UI（不限于探活）。

## 与既有 H1 关系

- 能力矩阵 H1：管「域要能做」。  
- **本 H1**：管「做的过程用户看得见 + 嘴上说的必须等于工具结果」。  
- 探活落库/UI 刷新已在 ✅ 探活 H1 / 0.3.63 回归；本条补 **可观测 Agent 体验**。
