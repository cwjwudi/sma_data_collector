...
流式只是 AiDrawer 体验的一环。对照当前抽屉（截图 + `AiDrawer.vue`），建议在本看板一并约定下列项；**默认标 ★ 的纳入流式同版或紧随小版，其余可另开切片**。

### 现状缺口（整窗）

| 点 | 现状 | 与流式关系 |
|----|------|------------|
| 正文展示 | `<pre>` 纯文本；Markdown 表格/标题以源码呈现 | 流式后更宜 **增量 Markdown 渲染**（半成品 fence 要容错） |
| 等待态 | 整包前仅「思考中…」；无「停止」 | ★ 流式中改为气泡增长 + **停止** 替换发送 |
| 工具轨迹 | 响应结束后一次性挂上 | ★ 流式中按 `tool` 事件追加；进行中可标「调用中」 |
| 阶段提示 | 无 | ★ `status` → 顶栏或气泡下「探活工具 / 撰写中」 |
| 并发发送 | `loading` 禁发送；无队列 | ★ **禁发 + 可停止**；不做排队（默认） |
| 清空 / 关抽屉 | 生成中仍可点（关抽屉不中止请求） | ★ 生成中清空需确认或禁用；关抽屉 **Abort**（默认） |
| Pending 弹框 | `AiPendingPromptDialog` 与聊天并行 | ★ 流未结束也可弹确认；确认后模型/下一轮再续（保持现架构） |
| 自动滚底 | 有（用户上翻可暂停） | ★ 流式 delta 时沿用同一策略 |
| 假失败标红 | 0.3.71 已修无 `ok` 误红 | 流式轨迹复用同一 `tool_result_ok` |
| 历史持久化 | 关抽屉/刷新丢失 | 本切片 **不做**（另案） |
| 重试 / 复制消息 | 无 | 本切片 **不做**（可选二期） |
| 抽屉宽度拖拽 | 固定宽 | 本切片 **不做** |
...
- 流式 **不改变** pending 确认语义（删模版、备份、结批等仍弹框）。  
- 页上下文仍随请求附带；抽屉内可用一行弱提示「已附带当前页：…」（可选，非必须）。
...
- [ ] 含工具调用：轨迹边到边；正文仍流式  
- [ ] **停止**可中止；已显示内容保留；关抽屉 Abort  
- [ ] 生成中不可重复发送；清空有保护  
- [ ] `insufficient_quota` 等仍中文友好  
- [ ] 探活空口答应仍被再调/改写，无假成功终态  
- [ ] （若同版 Markdown）表格/列表可读；未闭合代码块不崩  
...
- ✅ **整窗交互补遗**（停止/阶段/轨迹边到边/Markdown/关抽屉 Abort）  
- ⌛️ 实现与发版（待开工）
...
| Q5 | 生成中关抽屉 | **Abort 请求** |
| Q6 | 并发消息 | **禁发 + 停止**，不排队 |
| Q7 | Markdown 渲染 | **同版轻量渲染**（过大则顺延一小版） |
| Q8 | 会话持久化 / 重试按钮 | **不做**（另案） |
