...
| 停止 | **停止**只取消**当前**这一轮；队列里下一条默认仍继续（可选：停止时一并清队，见 Q6） |
| 关抽屉 Abort | 取消当前轮；**队列清空**（避免关着还在后台连发） |
| 清空 | 清消息 + **清队列** + Abort 当前 |
| Pending | 当前轮卡在确认框时，队列暂停；确认/取消后本轮收尾，再跑队首 |
...
| G5 | **停止 / 关抽屉 vs Pending** | 确认框还开着时 Abort | Abort 当前请求时 **一并取消该轮 pending**（与关抽屉清队一致） |
...
| G12 | **Esc** | Pending 已支持 Esc 取消 | 无 Pending 时 Esc **关抽屉**（生成中 = Abort+清队） |
...
| 清空 / 关抽屉 | 生成中可关且不中止 | ★ 清空有保护；关抽屉 **Abort + 清队** |
...
关抽屉/Esc：Abort 当前 + 清队 + 取消本轮 Pending（见 G5）
```
...
- [ ] 含工具：轨迹边到边；正文仍流式  
- [ ] **停止**可中止当前轮；关抽屉/Esc Abort + 清队 + 取消 Pending  
...
| Q14 | Abort 与 Pending | ★ **一并取消本轮 pending**（G5） |
| Q15 | Esc | ★ **关抽屉**（无 Pending 时）（G12） |
