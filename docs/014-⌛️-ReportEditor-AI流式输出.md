...
```text
空闲：     [清空] [发送]
生成中：   [清空需确认] [停止] [发送仍可用→入队]
有排队：   用户气泡旁「排队中」；当前 done 后自动发队首
停止当前： 当前气泡标「已停止」；默认继续跑队列下一条
关抽屉：   Abort 当前 + 清空队列
出错后：   本轮失败提示；默认仍尝试跑队列下一条（或暂停队列，见实现时定）
```
...
- [ ] 普通问答：正文流式出现  
- [ ] 含工具：轨迹边到边；正文仍流式  
- [ ] **停止**可中止当前轮；关抽屉 Abort + 清队  
- [ ] 生成中可再发：入队，前一条结束后自动连续答下一问  
- [ ] 关抽屉再开 / 刷新后**聊天仍在**（未点清空；排队中未发出的可丢弃或一并持久化，默认丢弃队列）  
- [ ] 边栏可拖宽并记住  
- [ ] Pending 确认框行为与现网一致  
...
| Q4 | 流式轨迹 UI | **边到边** |
| Q5 | 生成中关抽屉 | **Abort** |
| Q6 | 并发消息 | **可发入队 + 串行续答**（停止仅当前轮；关抽屉清队）← 已确认方向 |
