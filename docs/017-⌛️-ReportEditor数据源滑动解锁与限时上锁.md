...
### 方案 D · 琥珀安防 + 回退倒计时（最贴合已拍板交互）

![方案 D](assets/017-style-D-amber-countdown.png)

- **特征**：锁定琥珀轨 + 锁图标；解锁后绿系进度、**滑块回退**与「剩余 Ns」同框展示
- **气质**：安全态颜色清晰；直接画出 60s 回退语义
- **参考**：现有 `DatasourceLockToggle` 琥珀锁定色延伸 + 成熟确认滑条结构
- **顾虑**：色彩更花，需控制高度以免顶栏拥挤

**建议：D**（或 **C 的结构 + D 的琥珀锁定色**）——与已拍板「回退 + 秒数」最贴；若嫌花可选 C。

请回：**A / B / C / D**，或写「C 结构 + D 锁定色」等组合。
