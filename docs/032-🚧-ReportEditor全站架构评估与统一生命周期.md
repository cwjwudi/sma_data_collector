...
> **复评（2026-07-22 · 0.3.117）**：[034-🚧](034-🚧-ReportEditor全站架构复评-2026-07-22.md) — 综合 **≈3.7**；后续以 034 **改动清单 M1–M7** 为准（030 8k 零闪硬验收 **⏸ 挂起**）。
...
| ID | 目标 | 手段 | 验收 |
|----|------|------|------|
| P2-A | 导出与预览解耦 / 分机 | 专用渲染进程或报表机 | 同机 mappView 帧稳（030）；**分机近期不做** |
| P2-B | 旁路/更换 printToPDF | **0.3.114 选型+Spike 进行中** · [Plan](../_Prj/SD_SMA_ReportEditor/_Doc/009_版本Plan/0.3.114.md) · [030](030-🚧-ReportEditor结批占满CPU导致mappView白屏.md) | 弱 CPU：**零闪且 HMI 可操作**（030 硬验收） |
...
- 不逐页散修而不建 lifecycle。  
- ~~不在本轮换 PDF 引擎。~~ → **P2-B 已在 0.3.114 立项选型**（见 030）。  
- 不默认开工合并单元格（需产品拍板）。  
