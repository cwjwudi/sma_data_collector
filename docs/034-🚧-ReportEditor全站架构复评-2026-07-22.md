...
> **复评日期**：2026-07-22 · 代码线 **0.3.119**（M1–M7 行为测 + **M11 预览级 PDF 默认**）。  
...
| 导出 / 同机 HMI | **2** | **4** | **默认版式优先（预览级）**；同机优先标草稿非交付；030 零闪仍挂起 |
...
| **综合** | **≈3.0** | **≈4.1** | M1–M7+行为测+M11 默认预览级；030 零闪硬验收仍挂起 |

**核心问题已从「缺统一 pause / 草稿默认交付」转为**：同机零闪与预览级并存（M8–M10/M12）、030 硬验收解挂。
...
# 🚧 进行中：导出性能 4 档（收编 M8/M9 · 见 035）

> **主看板**：[035-🚧](035-🚧-ReportEditor导出性能档位与同机降载.md)（阶段 / 测试 / 开工计划）。  
> **拍板**：4 档滑条、默认 **均衡**；替代同机/版式双 Tab。

| 原 ID | 归入 035 | 说明 |
|-------|----------|------|
| **M8** | 阶段 D | 结批降载按档 `coexistPause` |
| **M9** | 阶段 D | 预热池按档 `prewarmPoolSize` |
| **M11 叙事** | 默认档 2 | 仍为 chromium 预览级；产品名改为性能档位 |
...
| ID | 改什么 | 说明 |
|----|--------|------|
| **035 A–F** | 导出性能 4 档 | 见 [035](035-🚧-ReportEditor导出性能档位与同机降载.md) |
| **M10** | 主进程冷路径去 sync | 可与 035 并行；不阻塞档位 |
...
| **M9** | → 035 | 见 035 | 预热按档 | 035 H4 |
| **M10** | 主进程冷路径去 sync | `electron/main.cjs` | 后端端口探测改 async `execFile`；备份读/日志写优先 `fs.promises` | 热路径契约仍绿 |
...
> CI 已覆盖契约 + 行为单测；下列**无法**用 vitest 替代。

## A. 预览级 PDF（M11 · 优先）

1. **新装/清 prefs 后**：生成报表 → 高级设置默认应为 **版式优先**（非「同机优先（草稿）」）。  
2. **旧库升级**：曾默认同机优先的本机，首次打开后应被一次性迁到版式优先；再手动切回「同机优先（草稿）」应能保留。  
3. **手动导出（版式优先）**：选含表格/占位符/仿宋的模版 → 导出 PDF → **对照编辑预览**：表格框、坐标、占位符插值、中文字体；页眉不应再出现 `fidelity=draft-v1` 纯文本流。  
4. **自动结批（版式优先）**：触发一条绑定 → 打开结果 PDF，同样对照预览。  
5. **草稿 opt-in**：切「同机优先（草稿）」导出一份 → 确认仍是 draft 观感，且 UI 标明不可交付（对照用，非验收交付）。
...
11. 同机有 mappView 时：用版式优先导出 1～2 份，**观察是否闪屏**（预期可能闪；记现象，硬验收仍挂起）。  
12. 审计/状态行：成功导出应出现 `版式优先` / `chromium` / `fidelity`（非 draft-v1）。
...
- **003**：推进序 = **M1–M7 ✅ → M11 ✅ → [035](035-🚧-ReportEditor导出性能档位与同机降载.md) 0.3.120 → 解挂 030 → P2**。  
- **030**：8k 零闪硬验收标 ⏸；同机降载与档位见 035。  
- **035**：导出性能 4 档（默认均衡）主看板。  
