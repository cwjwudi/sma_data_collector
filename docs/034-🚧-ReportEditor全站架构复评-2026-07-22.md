...
> **复评日期**：2026-07-22 · 代码线 **0.3.132**（M1–M7 / M11 ✅；**035 五档** + layout-v2 + 正文底色；本机 H1–H5 ✅）。  
...
| 主进程卫生 | **2.5** | **3.5** | 卷枚举/缩略图/写盘 async；端口探测等仍 sync（**M10**） |
...
| **综合** | **≈3.0** | **≈4.2** | M1–M7+M11+**035 五档（M8/M9）**；本机 H1–H5 ✅；030 零闪 / H6–H7 现场仍挂 |

**核心问题已从「缺统一 pause / 草稿默认交付 / 无性能档」转为**：**M10** 冷路径 sync、现场同机（035 H6/H7 + 030 解挂）、以及中长期 M12–M14。
...
| 当前优先 | **M1–M7 / M11 / 035（M8/M9）✅**；下一刀 **M10**；现场 **035 H6/H7** |
...
| ID | 改什么 | 说明 |
|----|--------|------|
| **M10** | 主进程冷路径去 sync | `electron/main.cjs`：端口探测改 async `execFile`；备份读/日志写优先 `fs.promises`；热路径契约仍绿 |
