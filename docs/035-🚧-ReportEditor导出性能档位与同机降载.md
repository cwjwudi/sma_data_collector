...
全量 8 万行虽只需查一次，但此前每条并行路都会把**整包**灌进窗内内存；用户指出「每份只需当份数据」。

## 改动

- 首份 fullSqlFill 后 `previewValuesForSplitReport` 按份写 `dir/part-N.json`
- 各窗 hydrate 只读当前 `reportPartIndex`；本窗缓存带 `partIndex`，换份必重灌
- Chromium 内存帽上调（约 15GB → 8 路）

## 验收

- 80 份 / maxRows=1000：并行路推进时各窗不应长期持有 8 万行
- 15GB + Chromium：遮罩约 8 路且各路能轮流完成（非只一路狂奔）
