...
- **036**：档 1 layout-v2 已同步填同一硬编码灰底（D1 ✅），两边一致但仍不可编辑；P1（D6/D8–D10）代码已落地，手测走应用内模拟结批

**需求**：页背景色（至少正文区，是否含眉脚另定）改为**可编辑**——模板或版式快照可配置，预览与档 2/3/4（及档 1）一致；默认可仍为白或现灰，但必须能改成纯白/`transparent`/自定义色。  
**状态**：⌛️ 仅记档，尚未实现。
