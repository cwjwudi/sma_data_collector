...
**用户复现时列表里已有「未正常连接 / 无法识别」的 OPC UA 配置。** 这一点是卡顿的关键触发条件，不是可选项：

- 本机当时（及 Agent 复测时）仍保留坏链：`666` → `opc.tcp://192.168.137.1:4840`（探活失败）。
- 同列表还有可达本机 demo：`WWW222` / `RRR` × 各 2 条重复（连点保存残留）。
- **没有坏链时**：`probeAll` 往往几十毫秒级，新建/删除几乎不卡 → 解释远端环境「登记了却复现不了」。
- **有坏链时**：每次保存 / 删除 / 切 tab 后的全量 `test_saved` 会被最慢那条坏链拖到约 **8s**。
...
## Agent 本机 API 复测（2026-08-11，对正在运行的 Report Editor AI :8000）

在用户环境直接打本机后端（未改业务代码；临时解锁 → 测完再锁回）：

| 步骤 | 结果 |
| ---- | ---- |
| 列表 | 5 条：坏链 `666@192.168.137.1:4840` + 可达重复 `WWW222`×2、`RRR`×2 |
| `test_saved` 坏链 `666` | **ok=false，约 8031ms** |
| 可达 4 条各自 | ok=true，约 17～32ms |
| 并行 `probeAll`（模拟 `loadServers` 后） | **墙钟约 8033ms**（被坏链封顶） |
| 无 id 连续 POST 新建同名 `052-repro-temp`×2 | **产生 2 个不同 uuid**（验证 A） |
| DELETE 一条临时配置 + GET + 再 `probeAll` | DELETE 本身 ~30ms，整链 **约 8078ms**（体感「删除卡顿」） |
| 清理 | 临时条目已删；数据源已重新锁定 |

结论复核：**「有未正常连接的 OPC UA」+ 每次 CRUD 后全量探活 = 新建/删除卡顿的充分条件**；连点无防重另制造重复条目与删除连打。

## 结论（给修复用）

1. **根因主链（OPC，已证实）**：无 id 连点保存 → 多 uuid；无 busy 连点删除 → 多次 DELETE + 审计爆发；保存/删除/切 tab → `probeAll` 全量测连，**未正常连接的配置超时拖垮交互**。  
...
- [x] 写明复现前置：**列表中存在未正常连接的 OPC UA**  
- [x] 写明与远端「不能复现」的环境差（坏链 + OPC 面板 + 连点）  
- [x] Agent 在用户本机对运行中后端复测：坏链 ~8s、`probeAll`/删除链 ~8s、无 id 双 POST 出双 uuid  
