...
- **DB Admin 千万级大库备份/恢复压测（计划）**：已建立 `feat/db-admin-large-backup-restore` 分支；计划仅读参考 `sma_data_test1`，在专用前缀测试库完成至少 3 轮千万级备份、断点下载、恢复与一致性验证，含约 12GB 容量场景；看板 [docs/025-⌛️](docs/025-⌛️-DBAdmin大库备份恢复压力测试.md)。
