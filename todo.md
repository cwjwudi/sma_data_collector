...
- **ReportEditor 048 需求登记（问题反馈包一键导出）**：现场一键打 zip——现象说明 + 环境元信息 + 相关模版 JSON +「生成报表」结批/操作配置 + 审计切片，便于交给 Cursor/Agent 复现排障；与全量 `.rebak`、纯审计导出区分；Q1–Q7 待拍板。看板 [docs/048-⌛️](docs/048-⌛️-ReportEditor问题反馈包一键导出.md)。
