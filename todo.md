...
- **QUERY_WEB 批次字典与互斥查询完成**：5000万行游标分页继续保留；新增无索引 Data_Batch（1000行）、批次下拉、Group `batch_field` 绑定，以及按时间/按批次号互斥校验。实库两类查询约12～14ms，79个非集成测试与浏览器联调通过。详见 [docs/022-✅](docs/022-✅-QUERY_WEB五千万行游标查询适配.md)。
