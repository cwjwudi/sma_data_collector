...
- **QUERY_WEB 批次来源配置与互斥查询完成**：5000万行游标分页继续保留；批次下拉来源表/字段改为 View 可配置，Group `batch_field` 负责业务字段映射；未配置时禁用批次模式。实库验证默认和不同命名来源，自动化测试及浏览器联调通过。详见 [docs/022-✅](docs/022-✅-QUERY_WEB五千万行游标查询适配.md)。
