...
- **表格系统缺陷修复（进行中）**：详见 [docs/002-🚧-表格系统评估与修复.md](docs/002-🚧-表格系统评估与修复.md)。已修并提交（TDD 先红后绿，前端全套 234 passed）：纵表/横表 0 行不再渲染字面 "…"（20dcdd4）；窄/多列表格列宽拖拽退化区不再反向/整体重置（d82bd57）；放开表格单行内容高度上限 240px→整页界避免长内容裁剪（6e7f792）。用户决策：240px＝放开上限+超页跨页断行（放开已做，行内跨页断行待排期）；P1-A＝预览与导出都按单报表上限、超出给提示不显示全量（待实现，注意后端预览 1000 行上限）。
