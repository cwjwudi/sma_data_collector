...
- **SD_SMA_DATA_COLLECTOR AA_test 24 小时稳定性测试**：连续运行 86400.141 秒，六组共采集 116384 个事件；三次 OPC UA 断线全部在 0.188 秒内恢复；六轮 PVI 扰动 42/42 写入成功且触发复位、六表均插入。程序内置 `passed=true`，快速测试 212 passed；窗口内发生一次约 13 分 55 秒的外部 MySQL 拒绝连接，重试队列最终清空且资源回落，因此恢复能力通过、严格零数据库错误稳定性不通过。看板与完整证据见 [docs/040-✅](docs/040-✅-数据采集器24小时稳定性测试.md)。
